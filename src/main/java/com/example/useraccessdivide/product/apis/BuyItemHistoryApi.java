...
public class BuyItemHistoryApi {
	...
	@GetMapping
	ResponseEntity<Pagingation<CartHistory>> viewHistory(long userId, int currentPage) {
		Page<CartHistory> cartHistoryEntityPage = cartHistoryService.findByUserId(userId, currentPage - 1, 2,
				Sort.by(Sort.Order.asc("boughtDatetime")));
		log.info("ABCSCS");
		return ResponseEntity.ok(new Pagingation<>(cartHistoryEntityPage.toList(),
				cartHistoryEntityPage.getTotalElements(), cartHistoryEntityPage.getTotalPages()));
	}
